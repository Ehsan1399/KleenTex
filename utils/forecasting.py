...
def choose_best_model_for_fold(train_series, validation_series):
    ...
    for model_name, func in candidates:
        try:
            ...
            penalty  = 0.0
            if hist_nzr<0.35 and pred_nzr>hist_nzr+0.25: penalty+= 0 # 5.0
            if pred_std==0.0 and val_std>0.0:             penalty+=0 #3.0
            #if demand_type=="intermittent" and model_name=="TSB": penalty-=0 # 1.0
            score = 0.5*mae+ 1*rmse+penalty
...
def score_all_models_for_fold(train_series, validation_series):
    ...
    for model_name, func in candidates:
        try:
            ...
            if pred_std == 0.0 and val_std > 0.0:               penalty += 0 #3.0
            #if demand_type == "intermittent" and model_name == "TSB": penalty -= 0 #1.0
            scores[model_name] = float(0.5*mae +  1*rmse + penalty)
        except Exception:
...
def run_forecast_pipeline(imputed_df, selected_skus, adi_threshold,
    ...
    for i, sku in enumerate(eligible_skus):
        ...
        description = (
            sku_df["Description"].dropna().astype(str).iloc[0]
            if "Description" in sku_df.columns and sku_df["Description"].notna().any()
            else ""
        )
        summary_rows.append({
            "SKU":                     sku,
