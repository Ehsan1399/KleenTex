...
def compute_sku_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for sku, grp in hist_df.groupby("SKU"):
        ...
        group = grp["Produc_Group"].dropna().astype(str).iloc[0]
        
        description = grp["Description"].dropna().astype(str).iloc[0] if "Description" in grp.columns and grp["Description"].notna().any() else ""
