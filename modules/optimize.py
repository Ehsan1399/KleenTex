...
def extract_results(model, I, T, demand_df, I_ini, Q, s, S, I_End, B, H, Area, F, C, desc_map=None, group_map=None):
    """
    ...
    for i in I:
        policy_rows.append({
            "Item":                  i,
            "Description":           desc_map.get(i, ""),
            "Group":                 group_map.get(i, ""),
...
def _run_heuristic_fallback(I, T, demand_df, I_ini, H, Area, F_val, C):
    ...
    obj_df = pd.DataFrame({
        "Objective Component": [
            "Z1: Inventory Cost", "Z2: Fixed Ordering Cost", "Total"],
...
def _run_optimization(data_dict, warehouse_cap, fixed_order, s):
    ...
    with st.spinner("🔧 Building and solving SCIP model…"):
        try:
            ...
            desc_map = {}
            fc_df = s.get("forecast_result")
            if isinstance(fc_df, pd.DataFrame) and "Description" in fc_df.columns:
                desc_map = (
                    fc_df[["SKU", "Description"]]
                    .dropna()
                    .drop_duplicates("SKU")
            ...
            if isinstance(raw_df, pd.DataFrame) and "SKU" in raw_df.columns and "Produc_Group" in raw_df.columns:
                group_map = (
                    raw_df[["SKU", "Produc_Group"]]
                    .dropna()
                    .drop_duplicates("SKU")
                    .assign(SKU=lambda x: x["SKU"].astype(str))
                    .set_index("SKU")["Produc_Group"]
                    .astype(str)
                    .to_dict()
                )
...
def _render_results(s):
    ...
    all_items = sorted(policy_df["Item"].astype(str).unique().tolist()) if not policy_df.empty else []

    if not policy_df.empty and "Group" in policy_df.columns:
        item_to_group = dict(zip(policy_df["Item"].astype(str), policy_df["Group"].astype(str)))
    else:
        item_to_group = {str(i): str(i)[:3].upper() for i in all_items}

    groups = ["All Groups"] + sorted(set(item_to_group.get(str(i), str(i)[:3].upper()) for i in all_items))
    ...
    with row1_l:
        with st.container(border=True):
            ...
            with cs:
                items_in_grp = (
                    [i for i in all_items if item_to_group.get(str(i), str(i)[:3].upper()) == grp_sel]
                    if grp_sel != "All Groups" else all_items
                )
                sku_display = [f"{i} ({item_to_group.get(str(i), str(i)[:3].upper())})" for i in items_in_grp]
    ...
    if sel_item and not policy_df.empty and not plan_df.empty:
        ...
        holding_sku  = round(rows_obj.get("Z1: Inventory Cost", 0) * sku_frac, 2)
        ordering_sku = round(rows_obj.get("Z2: Fixed Ordering Cost",  0) * sku_frac, 2)
    ...
    with row2_r:
        if has_sku_data:
            ...
            z1_total      = round(rows_obj_all.get("Z1: Inventory Cost", 0), 2)
            z2_total      = round(rows_obj_all.get("Z2: Fixed Ordering Cost",  0), 2)
        ...
        for _, row in tbl_plan[display_cols].iterrows():
            ...
            for c in display_cols:
                ...
                if c == "Item":
                    ...
                    cells += f'<td style="{_TD_NUM}text-align:left;font-family:DM Sans,sans-serif;">{val}</td>'
