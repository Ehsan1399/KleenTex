...
def render():
    ...
    for _, row in tbl_data.iterrows():
        ...
        rows_html += (
            ...
            f'<td style="{td}">{row.get("Description", "")}</td>' +
...
def _render_sku_selector(df, skus, n_skus, key_suffix=""):
    ...
    with col_a:
        ...
        selection_method = st.selectbox(
            "Selection method",
            ["Top N by Revenue", "Top N by Demand", "Manual Selection"],
    ...
    with col_b:
        ...
        if selection_method == "Manual Selection":
            ...
            preview_metric = pd.Series(
                data=[1] * len(selected_skus),
                index=selected_skus
            )
            ...
            if selection_method == "Top N by Revenue":
                if "Monthly_Sales_EUR" not in df.columns:
                    st.warning("Monthly_Sales_EUR column not found. Falling back to Monthly_QTY.")
                    sku_metric = df.groupby(grp_col)["Monthly_QTY"].sum()
                    col_label = "Total Demand"
                else:
                    sku_metric = df.groupby(grp_col)["Monthly_Sales_EUR"].sum()
                    col_label = "Revenue"

            elif selection_method == "Top N by Demand":
                sku_metric = df.groupby(grp_col)["Monthly_QTY"].sum()
                col_label = "Total Demand"

            else:
                sku_metric = df.groupby(grp_col)["Monthly_QTY"].count()
                col_label = "Data Points"

            top_skus = sku_metric.sort_values(ascending=False).head(int(n_select)).index.astype(str).tolist()
            selected_skus = top_skus
            preview_metric = sku_metric.loc[top_skus]
        ...
        display_skus = (
            [sk for sk in selected_skus if search_val.lower() in sk.lower()]
            if search_val else selected_skus
        )

        _th = (
            "padding:10px 14px;text-align:left;font-size:16px;text-transform:uppercase;"
            "letter-spacing:.09em;color:#6b859e;font-weight:500;border-bottom:1px solid #243b55;"
            "white-space:nowrap;"
        )
        _td_sku = (
            "padding:11px 14px;font-size:16px;color:#f4f8fb;border-bottom:1px solid #1a2e45;"
            "font-family:DM Mono,monospace;font-weight:500;"
        )
        _td_num = (
            "padding:11px 14px;font-size:16px;color:#a8c0d4;border-bottom:1px solid #1a2e45;"
        ...
        rows_html = "".join(
            ...
            f'<td style="{_td_num}">{round(float(preview_metric.get(sk, 0)), 2)}</td>'
            f'</tr>'
        ...
        table_html = (
            ...
            f'<strong style="color:#f4f8fb">{len(selected_skus)}</strong> selected SKUs</div>'
