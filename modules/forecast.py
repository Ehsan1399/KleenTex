...
def render():
    ...
    df_imp_selected = df_imp[df_imp["SKU"].astype(str).isin([str(x) for x in sel_skus])].copy()

    all_skus_imp = sorted(df_imp_selected["SKU"].dropna().astype(str).unique())

    stats_df_tmp = compute_sku_stats(df_imp_selected)
    sku_to_group = dict(zip(stats_df_tmp["SKU"].astype(str), stats_df_tmp["Group"].astype(str)))

    groups_imp = ["All Groups"] + sorted(set(sku_to_group.get(sk, sk[:3].upper()) for sk in all_skus_imp))
    ...
    with col_k:
        skus_in_grp = (
            [sk for sk in all_skus_imp if sku_to_group.get(sk, sk[:3].upper()) == grp_param]
            if grp_param != "All Groups" else all_skus_imp
        )
        sku_labels = {sk: f"{sk} ({sku_to_group.get(sk, sk[:3].upper())})" for sk in skus_in_grp}
    ...
    if has_fc and sku_sel:
        ...
        if not sku_data.empty:
            ...
            grp_lbl = sku_to_group.get(sku_sel, sku_sel[:3].upper())
